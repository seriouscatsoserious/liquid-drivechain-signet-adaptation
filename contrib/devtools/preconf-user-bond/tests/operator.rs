mod common;
use common::{collateral, key, sign, sighash, transfer};
use common::operator::*;
use elementsplus_preconf::{elements::{self, confidential, hashes::Hash, AssetId, BlockHash, Sequence, TxOut, Txid},
    operator::{penalty_action, refund_action, Receipt}, simplicity::jet::elements::ElementsUtxo};

#[test]
fn preconfer_alone_is_accountable_and_principal_is_not_spent() {
    let (a,b) = evidence();
    bond().verify(&a).unwrap(); bond().verify(&b).unwrap();
    for (first,second) in [(&a,&b),(&b,&a)] {
        let env = penalty_env();
        let stack = bond().satisfy(&env, &penalty_action(first,second).unwrap()).unwrap();
        ...
        assert_eq!(hex::encode(&stack[3][1..]),
            "50929b74c1a04954b78b4b6035e97a5e078a5a0f28ec96d547bfee9ace803ac0");
        assert_ne!(env.tx().input[0].previous_output,config().protected_output);
        assert!(env.tx().output[0].is_fee());
        assert_eq!(env.tx().output[0].value,confidential::Value::Explicit(config().collateral));
    }
}
...
fn every_operator_signature_is_required_and_user_signature_cannot_substitute() {
    for which in 0..4 {
        ...
        assert!(bond().satisfy(&penalty_env(),&penalty_action(&a,&b).unwrap()).is_err());
    }
}
...
fn same_transaction_is_not_slashable_even_with_randomized_signature() {
    ...
    use simplicityhl::{types::{ResolvedType,TypeConstructible}, value::{UIntValue,Value,ValueConstructible}};
    let (a,mut b) = evidence();
    b.txid = a.txid;
    b.signature = *Secp256k1::new().sign_schnorr_with_aux_rand(
        &Message::from_digest(bond().digest(b.bond,b.txid)),&key(2),&[9;32]).as_ref();
    ...
    let promise = |r:&Receipt| Value::tuple([UIntValue::U256(simplicityhl::num::U256::from_byte_array(r.txid.to_byte_array())).into(),Value::byte_array(r.signature)]);
    let action = Value::left(Value::tuple([promise(&a),promise(&b)]),ResolvedType::array(ResolvedType::u8(),64));
...
fn domain_chain_session_and_outpoint_replay_rejected() {
    ...
    for field in 0..10 {
        ...
        if field==9 { tx.input[0].previous_output.vout += 1; }
        assert!(other.satisfy(&env_for(&other,tx),&action).is_err(),"field {field}");
    }
    let env = bond().environment(penalty_env().tx().clone(),vec![bond().funding_output().into()],
        BlockHash::from_byte_array([0x77;32])).unwrap();
...
fn diversion_partial_burn_extra_io_and_asset_substitution_rejected() {
    ...
    for attack in 0..9 {
        ...
        match attack {
            ...
            3 => tx.output[0].asset = confidential::Asset::Explicit(AssetId::from_byte_array([0x99;32])),
        ...
        assert!(bond().satisfy(&env_for(bond(),tx),&action).is_err(),"attack {attack}");
    }
}
...
fn invalid_collateral_and_principal_consumption_rejected() {
    ...
    for attack in 0..6 {
        ...
        match attack {
            ...
            4 => output.asset = confidential::Asset::new_confidential(
                &elements::secp256k1_zkp::Secp256k1::new(),config().fee_asset,
                confidential::AssetBlindingFactor::from_slice(&[1;32]).unwrap()),
            _ => output.value = confidential::Value::new_confidential_from_assetid(
                &elements::secp256k1_zkp::Secp256k1::new(),config().collateral,config().fee_asset,
                confidential::ValueBlindingFactor::from_slice(&[2;32]).unwrap(),
                confidential::AssetBlindingFactor::from_slice(&[1;32]).unwrap()),
        }
        let env = bond().environment(penalty_env().tx().clone(),vec![ElementsUtxo::from(output)],config().genesis).unwrap();
    ...
    let a = receipt(output,transfer(1).txid()); let b = receipt(output,transfer(2).txid());
    let mut tx = penalty_env().tx().clone(); tx.input[0].previous_output = output;
    assert!(bond().satisfy(&env_for(bond(),tx),&penalty_action(&a,&b).unwrap()).is_err());
}
...
fn only_operator_can_refund_after_deadline_and_signature_commits_to_outputs() {
    ...
    assert!(bond().satisfy(&env,&refund_action(&sign(sighash(&env),1))).is_err());
    for attack in 0..3 {
        let mut tx = tx.clone();
        match attack {
            0 => tx.lock_time = elements::LockTime::from_height(config().refund_height-1).unwrap(),
        ...
        let action = if attack<2 { refund_action(&sign(sighash(&env),2)) } else {refund_action(&sig)};
        assert!(bond().satisfy(&env,&action).is_err());
    }
}
...
fn wire_format_is_canonical_and_preserves_epoch_precision() {
    let mut c = config(); c.epoch=u64::MAX;
    let json = serde_json::to_string(&c).unwrap();
    assert!(json.contains("\"epoch\":\"18446744073709551615\""));
    assert_eq!(serde_json::from_str::<elementsplus_preconf::operator::Config>(&json).unwrap().epoch,u64::MAX);
    let (r,_) = evidence();
